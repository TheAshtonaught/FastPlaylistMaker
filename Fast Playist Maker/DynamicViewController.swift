...
//
struct _Section {
  var name: _ParamTypes
  var items: [_Params]
  var collapsed: Bool

  init(name: _ParamTypes, items: [_Params], collapsed: Bool = true) {
    self.name = name
    self.items = items
...
class DynamicViewController: UITableViewController {
  ...
  var dictionary = [_Params: UITextField]()
  var longLink: URL?
  var shortLink: URL?
  ...
  override func viewDidLoad() {
    ...
    sections = [
      _Section(name: .googleAnalytics, items: [.source, .medium, .campaign, .term, .content]),
      _Section(name: .iOS, items: [.bundleID, .fallbackURL, .minimumAppVersion, .customScheme,
                                  .iPadBundleID, .iPadFallbackURL, .appStoreID]),
      _Section(name: .iTunes, items: [.affiliateToken, .campaignToken, .providerToken]),
      _Section(name: .android, items: [.packageName, .androidFallbackURL, .minimumVersion]),
      _Section(name: .social, items: [.title, .descriptionText, .imageURL])
    ]
  }

  func buildFDLLink() {
    // [START buildFDLLink]
    if DynamicViewController.DYNAMIC_LINK_DOMAIN == "YOUR_DYNAMIC_LINK_DOMAIN" {
...
//
extension DynamicViewController : UIGestureRecognizerDelegate {
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    default:
      ...
      if row == 0 {
        ...
        cell.toggleButton.addTarget(self, action: #selector(DynamicViewController.toggleCollapse), for: .touchUpInside)
        return cell
      } else {
        let cell = tableView.dequeueReusableCell(withIdentifier: "param", for: indexPath) as! ParamTableViewCell
        cell.paramLabel.text = sections[section].items[row - 1].rawValue
        if cell.paramLabel.text! == _Params.bundleID.rawValue {
          cell.paramTextField.text = Bundle.main.bundleIdentifier
        } else if cell.paramLabel.text! == _Params.minimumAppVersion.rawValue {
          cell.paramTextField.text = "1.0"
        }

        dictionary[_Params(rawValue: cell.paramLabel.text!)!] = cell.paramTextField
        return cell
      }
    }
  }
